import {selectDiverseRound,hasRoundDiversity,categoryGroup} from './round-diversity.js';
...
export class GameAI {
  ...
  async round(exclude=[]){
    ...
    for(let attempt=0;attempt<2&&!hasRoundDiversity(questions);attempt++){
      ...
      const represented=new Set(questions.map(categoryGroup));const focus=[...topics].sort(()=>Math.random()-.5).sort((a,b)=>Number(represented.has(categoryGroup({category:a})))-Number(represented.has(categoryGroup({category:b})))).slice(0,5);
      const data=await this.call(GENERATE_SYSTEM,{count:10,requirements:'生成10道候选题，严格按focus中的五个方向各生成两题，category必须使用对应focus的完整名称，不得全部属于同一方向。影视相关内容都属于同一大类，不能靠更换标签充当不同类别。优先不同于排除列表的知识集合。不要回到国家、省份、自治区等已列出题目。仅补充新题，不要返回排除列表中的题。',focus,exclude:[...used,...questions.map(q=>q.title)]},7500);
      ...
      for(const candidate of data.questions){
        ...
        const key=titleKey(q.title);if(blocked.has(key))continue;blocked.add(key);q.id='ai-'+crypto.randomUUID();candidates.push(q);
      }
      questions=selectDiverseRound(candidates);
    }
    if(!hasRoundDiversity(questions))throw new AIError('两次生成仍未凑齐至少五类的七道有效新题，原有游戏已保留。请稍后重试；本次没有替换成题库题。');
