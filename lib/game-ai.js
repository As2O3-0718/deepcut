...
export class GameAI {
  ...
  async round(exclude=[]){
    ...
    const data=await this.call(GENERATE_SYSTEM,{count:10,requirements:'生成10道候选题，覆盖至少5类。系统会去重后选择7题。不要重复以下题目，请探索不同知识集合。',exclude:used},6500);
    ...
    // Keep good generated questions; fill the remainder without another paid call.
    const excluded=new Set([...exclude,...questions.map(q=>q.title)].map(titleKey));
    for(const q of pickRound(bank.filter(q=>!excluded.has(titleKey(q.title))))){
      if(questions.length===7)break;const key=titleKey(q.title);if(excluded.has(key))continue;excluded.add(key);questions.push({...q,roundFallback:true});
    }
    if(questions.length!==7)throw new AIError('可用的不重复题目不足七道，请恢复部分屏蔽题目后重试。');
    for(const q of questions)if(!q.roundFallback)this.dynamic.set(q.id,q);
